...
DatetimePicker.prototype = {
    ...
    if (self.container) {
      ...
      each(TYPE_MAP, function (type) {
        ...
        if (type === 'day') {
          ...
          var startYear = self.config.minYear
          var startMonth = self.config.minMonth
          data = self._makeData(type, startYear, startMonth)
        }
        ...
        self[type + 'Scroller'] = renderScroller(div, data, trimZero(newValueMap[type]), function (currentValue) {
          ...
          if (type === 'year') {
            ...
            self._setMonthScroller(currentValue, currentMonth, currentDay)
  ...
  _makeData (type, year, month) {
    ...
    if (type === 'year') {
      ...
      if (config.minYear == config.maxYear) {
        if (config.minMonth == config.maxMonth) {
          min = config.minDay
          max = config.maxDay
        } else {
          if (month == config.maxMonth) {
            min = 1
            max = config.maxDay
          } else if (month == config.minMonth) {
            min = config.minDay
  ...
  _setDayScroller (year, month, day) {
    ...
    if (year == self.config.maxYear) {
      if (month >= self.config.maxMonth) {
        month = self.config.maxMonth
        day = self.config.maxDay
      }
    }else if (year == self.config.minYear) {
      if (month <= self.config.minMonth) {
        month = self.config.minMonth
        day = self.config.minDay
      }
    }
  ...
  _setMonthScroller (year, month, day) {
    var self = this
    if (year == self.config.maxYear) {
      if (month > self.config.maxMonth) {
        month = self.config.maxMonth
    ...
    self.monthScroller = renderScroller(div, self._makeData('month', year, month), month, function (currentValue) {
